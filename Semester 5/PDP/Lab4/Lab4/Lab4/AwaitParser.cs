...
namespace Lab4
{
    public class AwaitParser
    {
        class Session
        {
            ...
            {
                Console.WriteLine($"Await: {message} {_filePath.Substring(_filePath.LastIndexOf("\\") + 1)} {Thread.CurrentThread.ManagedThreadId}");
            }
            public Session(Socket conn, string filePath) {
                _conn = conn;
                _buffer = new byte[1024];
                _filePath = "E:\\University\\Semester 5\\PDP\\Lab4\\Lab4\\Lab4\\downloads\\await\\" + filePath + ".txt";
        ...
        public AwaitParser(List<string> urls)
        {
