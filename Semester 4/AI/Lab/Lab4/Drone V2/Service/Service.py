...
class Service:
    ...
    def epoch(self, graph, start, end, ants):
        run = True
        while run:
            run = False
            for ant in ants:
    ...
    def solverPath(self, graph, start, end, ants):
        bestAnt = None

        for epoch in range(EPOCHS):
            ant = self.epoch(graph, start, end, ants)
    ...
    def solver(self):
        ...
        ants = [Ant(self._drone.position, closeSensor, graph) for _ in range(NO_ANTS)]
        firstPath = self.solverPath(graph, self._drone.position, closeSensor, ants)[0]
        ...
        for i in range(len(sensors) - 1):
            ...
            for destination in sensors[i + 1:]:
                ...
                ants = [Ant(sensor, destination, mapGraph) for _ in range(NO_ANTS)]
                paths[(sensor, destination)] = self.solverPath(mapGraph, sensor, destination, ants)[0]
                paths[(destination, sensor)] = paths[(sensor, destination)]

        self._sensorGraph.initGraph(paths)

        ants = [CycleAnt(closeSensor, _, self._sensorGraph) for _ in range(NO_ANTS)]
        cycle = self.solverPath(self._sensorGraph, closeSensor, None, ants)[0]
