...
WHERE B.LibraryId IN
(SELECT LibraryId
FROM Libraries
WHERE LibraryName = 'Biblioteca Judeteana Mihai Eminescu' OR LibraryName = 'Biblioteca Centrala Universitara Lucian Blaga'
)
...
-- Get the top 50 percent most visited libraries in Transilvania
...
INNER JOIN (
	...
	WHERE B.Pages >= 400
) TL ON TT.TitleId = TL.TitleId
WHERE TL.LibraryName = 'Biblioteca Centrala Universitara Lucian Blaga'
ORDER BY TL.Title

-- Show the borrowers cities and the books they have borrowed together with the cities and the libraries in which they are in and the start and end date of the borrowing

SELECT BC.FullName, BC.CityName 'As borrower city', BTC.Title, BTC.CityName AS 'Book city', BTC.LibraryName AS 'Book library', FORMAT(BB.StartDate, 'dd-MM-yyyy') AS 'Start Date', FORMAT(BB.EndDate, 'dd-MM-yyyy') AS 'End Date'
FROM Borrowers_Books BB
INNER JOIN (
	...
	INNER JOIN Cities C ON L.CityId = C.CityId
) AS BTC ON BTC.BookId = BB.BookId
INNER JOIN (
...
) BC ON BB.BorrowerId = BC.BorrowerId
...
-- Get the authors with the most books written which have written more books than Paulo Coelho
