...
-- Show the borrowers cities and the books they have borrowed together with the cities and the libraries in which they are in and the start and end date of the borrowing and the number of days borrowed

SELECT BC.FullName, BC.CityName 'As borrower city', BTC.Title, BTC.CityName AS 'Book city',BTC.LibraryName AS 'Book library',
FORMAT(BB.StartDate, 'dd-MM-yyyy') AS 'Start Date', FORMAT(BB.EndDate, 'dd-MM-yyyy') AS 'End Date', DATEDIFF(DAY,BB.StartDate, BB.EndDate) AS 'Number of days borrowed'
FROM Borrowers_Books BB
INNER JOIN (
